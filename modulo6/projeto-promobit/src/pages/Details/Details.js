...
import {Content, CastCard, CastName, CrewInfo, Synopsis, CrewTitle, CastTitle, DivContainer, MovieInfo,
  Assessment, MovieCover, MovieTitle, CrewContent, MovieDetail, MidiaContent, MovieTrailer,
...
export default function MovieDetails(params) {
  ...
  const [movieRecmd, setMovieRecmd] = useState([])
  ...
  useEffect(() => {
    try {
      ...
      axios.get(`${BASE_URL}${id}/recommendations?${API_KEY}&language=pt_BR&page=1`)
        .then(response => setMovieRecmd(response.data.results))
    } catch (error) {
      console.log(error)
    }
  }, [id, movieCurrent])
  ...
  return (
    ...
    <DivContainer>
      ...
      <Content>
        <MovieCover
          src={`${BASE_URL_IMAGE}original/${poster_path}`}
          alt={title}
        />
        <MovieDetail>
          <MovieTitle>
              ...
              <img
                src={
                  cast.profile_path === null
                    ? Error
                    : `${BASE_URL_IMAGE}original/${cast.profile_path}`
                }
                alt={cast.name}
              />
        ...
        <MovieTrailer>
          {videos && videos.results.length ? (
            <ReactPlayer
              width="100%"
              height="100%"
              url={`${BASE_URL_YT}${videos ? videos.results[0].key : ''}`}
              controls={true}
            />
          ) : (
            <div>
